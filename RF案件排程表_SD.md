...
**權限要求:**
- 新增法規: `REGULATION_ADD`
- 停用法規: `REGULATION_DISABLE`
- 移除法規: `REGULATION_REMOVE`

**業務規則:**
1. 停用法規: 設定 ManualStatusOverride = true, Status = OnHold
2. 移除法規: 執行 Soft Delete (需確認無測項)
3. 編輯法規: 可修改名稱、日期、備註

---

### 6.3.4 新增法規對話框 **[v4.0 新增 - SCR-REGULATION-002]**
...
### 6.3.5 停用/移除法規對話框 **[v4.0 新增 - SCR-REGULATION-003]**
...
```json
// 停用法規
PUT /api/v1/regulations/123/disable
{
  "reason": "客戶暫時取消此認證需求"
}

// 移除法規
DELETE /api/v1/regulations/123
{
  "reason": "專案變更,不再需要此法規"
}
```
...
### 6.3.6 測項列表介面 **[v4.0 新增 - SCR-TESTITEM-LIST-001]**
...
│                                                             │
│  ┌───────────────────────────────────────────────────── GridControl
│  │測項名稱          │類型    │地點│預估h│狀態  │負責人│操作  │
│  ├─────────────────────────────────────────────────────┤
│  │Conducted Emission│Conduct│LabA│40.0│InProg│張三  │[詳][編]│
│  │Radiated Spurious │Radiate│LabB│60.0│InProg│李四  │[詳][編]│
│  │Blocking Test     │Blocking│LabA│30.0│NotSt│張三  │[詳][編]│
│  └─────────────────────────────────────────────────────┘
│                                                             │
│  操作說明:                                                   │
│  [詳]: 查看測項詳情(含工程師分配、補測版本、工時記錄)          │
│  [編]: 編輯測項資訊                                           │
│                                                             │
└───────────────────────────────────────────────────────────┘
```
...
- 顯示指定法規下的所有測項
- 支援新增、編輯測項
- 查看測項詳情(工程師分配、補測版本、工時記錄)
- GridControl 支援排序、篩選

**權限要求:**
- 新增測項: `TESTITEM_CREATE`
- 編輯測項: `TESTITEM_EDIT`
- 查看詳情: `TESTITEM_VIEW_ALL`

---

### 6.3.7 工程師指派介面 **[v4.0 新增 - SCR-ENGINEER-ASSIGN-001]**

**佈局:**
```
┌──────────────────────────────────────────
│  工程師指派 - Conducted Emission   [X]    │
├──────────────────────────────────────────┤
│                                          │
│  測項: Conducted Emission                │
│  預估工時: 40.0 小時                      │
...
**業務規則:**
1. 必須至少指派1位Main1
2. 最多可指派3位Main(Main1/Main2/Main3)
3. Support人數不限
4. 系統檢查工程師Loading,超過100%時警告
5. 建議工時分配總和等於預估工時

**權限要求:**
- 分配主要工程師: `TESTITEM_ASSIGN_ENGINEER`
- 分配支援工程師: `TESTITEM_ASSIGN_SUPPORT`
- 移除工程師: `TESTITEM_REMOVE_ENGINEER`

**操作流程:**
1. 選擇工程師
2. 設定角色類型 (Main1/Main2/Main3/Support)
3. 輸入分配工時
4. 系統檢查 Loading
5. 確認指派
...
### 6.3.8 補測版本管理介面 **[v4.0 新增 - SCR-REVISION-LIST-001]**

**佈局:**
```
┌───────────────────────────────────────────────────────────
│  補測版本管理 - Conducted Emission              [X]          │
├───────────────────────────────────────────────────────────┤
│                                                             │
│  [+ 建立補測版本]                                            │
│                                                             │
│  ┌───────────────────────────────────────────────────── GridControl
│  │版本│類型    │預估h│建立日期  │原因            │狀態│操作  │
│  ├─────────────────────────────────────────────────────┤
│  │v1  │原始版本│40.0│2025/11/20│-              │已完成│-   │
│  │v2  │Command│20.0│2025/12/01│客戶要求重測    │進行中│[回滾]│
│  │v3  │Retest │15.0│2025/12/10│測試結果不符    │未開始│[回滾]│
│  └─────────────────────────────────────────────────────┘
│                                                             │
│  版本說明:                                                   │
│  • v1: 原始版本(TestItem建立時)                              │
│  • v2+: 補測版本(記錄於TestItemRevision表)                  │
│                                                             │
│  ⚠ 回滾版本將標記該版本為IsDeleted,已回報工時保留           │
│                                                             │
└───────────────────────────────────────────────────────────┘
```

**權限要求:**
- 建立補測版本: `TESTITEM_REVISION_CREATE`
- 回滾版本: `TESTITEM_REVISION_ROLLBACK`
...
│                                          │
│  補測原因* (最多200字):                  │
│  [測試結果在2.4GHz頻段超標3dB,需要___]   │
│  [重新測試確認是否為測試環境問題______]   │
│  [________________________________]      │
│                                          │
│  詳細說明 (最多500字):                   │
│  [經檢查發現可能是PCB佈局問題,客戶已___] │
│  [提供新版樣品,需要重新進行Conducted__]   │
│  [Emission測試___________________]      │
│                                          │
│  ⚠ 建立補測版本後,測項狀態將自動變為InProgress │
│  ⚠ 相關工程師將收到通知                  │
│                                          │
│  [取消]              [確定建立]           │
│                                          │
...
**系統行為:**
1. 自動計算新版本號 (v2, v3, v4...)
2. 建立 TestItemRevision 記錄
3. TestItem.Status 自動變為 InProgress
4. 通知相關工程師
5. 記錄 AuditLog
6. 觸發狀態重算
...
POST /api/v1/testitems/123/revisions
{
  "revisionType": "Retest",
  "estimatedHours": 15.0,
  "reason": "測試結果在2.4GHz頻段超標3dB,需要重新測試確認是否為測試環境問題",
  "description": "經檢查發現可能是PCB佈局問題,客戶已提供新版樣品,需要重新進行Conducted Emission測試"
}
```
...
**權限要求:**
- 覆寫測項狀態: `TESTITEM_STATUS_OVERRIDE`

**業務規則:**
1. 必須填寫覆寫理由
2. 設定 ManualStatusOverride = true
3. 自動狀態計算將被停用
4. 記錄完整的 AuditLog
5. 觸發 Regulation 狀態重算
...
│  │11/17│王五  │Conducted Emission│8.0 │進行中│[編輯] │  │
│  │     │      │                  │    │      │       │  │
│  └─────────────────────────────────────────────────────     │
│                                                             │
│  ⚠ = 異常工時 (>10h 或 <1h)                                 │
...
**權限要求:**
- 修改工時: `WORKLOG_EDIT_ALL`

**業務規則:**
1. 主管可修改任何工時記錄
2. 必須填寫修改理由
3. 記錄 ModificationReason
4. 發送通知給工程師
5. 記錄完整的 AuditLog
6. 可能觸發測項狀態重算
...
**API 呼叫:**
```json
PUT /api/v1/worklogs/456
{
  "actualHours": 8.0,
  "status": "InProgress",
  "modificationReason": "工程師誤報工時,實際為8小時"
}
```
...
│                                                             │
│  團隊統計:                                                   │
│  平均Loading: 78.3%  |  超載人數: 1人  |  閒置人數: 0人      │
│                                                             │
└───────────────────────────────────────────────────────────┘
```
...
**Loading 計算:**
```
已分配工時 = Σ(所有 Active 專案中分配給該工程師的工時)
實際工時 = Σ(該工程師實際回報的工時)
Loading% = 已分配工時 / 本週可用工時 × 100%
```
...
**權限要求:**
- 查看所有工程師Loading: `LOADING_VIEW_ALL`
- 調整可用工時: `USER_MANAGE`
...
│  │Proj A │Radiated Emission │張三  │2      │客戶延遲  │  │
│  │       │                  │      │       │          │  │
│  └─────────────────────────────────────────────────────     │
│                                                             │
│  === 延遲趨勢 ===                                            │
...
│                                                             │
│  統計摘要:                                                   │
│  總延遲次數: 52次  |  平均延遲天數: 4.2天  |  受影響專案: 8個│
│  本月較上月: ↑ 12% (需關注)                                 │
│                                                             │
└───────────────────────────────────────────────────────────┘
```
...
**統計維度:**
- 依原因類型統計 (Equipment/Customer/Engineer/Location/Other)
- 依專案統計
- 依工程師統計
- 依時間趨勢統計
...
│  │日期      │原因          │天數│備註│   │
│  ├────────────────────────────────────┤   │
│  │11/18-20 │測試設備故障  │3   │頻譜│   │
│  │         │              │    │分析│   │
│  │         │              │    │儀故│   │
│  │         │              │    │障  │   │
│  │11/15-16 │客戶延遲樣品  │2   │新版│   │
│  │         │              │    │PCB │   │
│  └────────────────────────────────────   │
│                                          │
│  累計延遲: 5天                           │
...
**權限要求:**
- 查看延遲分析: `REPORT_VIEW_ALL`

**API 呼叫:**
```json
GET /api/v1/reports/delay-analysis?startDate=2025-11-01&endDate=2025-11-30&projectId=100
```
...
│                                          │
│  停用原因*: (必填)                       │
│  [員工離職___________________________]  │
│  [________________________________]      │
│                                          │
│  ⚠ 停用後該用戶將無法登入                │
│  ⚠ 歷史資料將保留                        │
...
|------|---------|------|
| 6.3.3 | 法規管理介面 | ✅ v4.0新增 |
| 6.3.4 | 新增法規對話框 | ✅ v4.0新增 |
| 6.3.5 | 停用/移除法規對話框 | ✅ v4.0新增 |
| 6.3.6 | 測項列表介面 | ✅ v4.0新增 |
| 6.3.7 | 工程師指派介面 | ✅ v4.0新增 |
| 6.3.8 | 補測版本管理介面 | ✅ v4.0新增 |
| 6.3.9 | 建立補測版本對話框 | ✅ v4.0新增 |
| 6.3.10 | 測項狀態管理介面 | ✅ 已定義 |
| 6.3.11 | 工時審核與覆寫介面 | ✅ 已定義 |
| 6.3.12 | Loading監控介面 | ✅ 已定義 |
| 6.3.13 | 延遲分析介面 | ✅ 已定義 |
| 6.3.14 | 用戶管理介面 | ✅ 已定義 |
